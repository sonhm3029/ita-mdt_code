...
class DressCodeDataset(Dataset):
    def __init__(
        ...
        c_names = []
        if self.is_test:
            with open(opj(self.drd, f"{self.data_type}_pairs_{self.pair_key}.txt"), "r") as f:
        ...
        self.c_names = dict()
        self.c_names["paired"] = [name.replace('_0', '_1') for name in im_names]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_size is None and self.transform_color is None:
            ...
            vit_cloth = imread(
                opj(self.drd, "images", self.c_names[self.pair_key][idx]), 
                self.vit_img_H, 
                self.vit_img_W
            )

            image = imread(opj(self.drd, "images", self.im_names[idx]), self.img_H, self.img_W)
            image_densepose = imread(opj(self.drd, "image-densepose", self.im_names[idx]), self.img_H, self.img_W)

        else:
            agn = imread_for_albu(opj(self.drd, "agnostic", self.im_names[idx]))
