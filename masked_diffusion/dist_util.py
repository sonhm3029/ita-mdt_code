...
def load_and_broadcast_state_dict(state_dict, **kwargs):
    """
    Broadcast a PyTorch model state dictionary across MPI ranks without redundant fetches.
    """
    chunk_size = 2 ** 30  # MPI has a relatively small size limit
    if MPI.COMM_WORLD.Get_rank() == 0:

        buffer = io.BytesIO()
        th.save(state_dict, buffer)
        buffer.seek(0)
        data = buffer.read()
